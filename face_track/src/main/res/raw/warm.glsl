...
{ 
	...
	lowp vec4 face.camera.beans.base = vec4(mix(textureColor.rgb, textureColor2.rgb, 1.0 - greyColor.r), textureColor.a);
	lowp vec4 overlayer = vec4(layerColor.r, layerColor.g, layerColor.b, 1.0);

    // screen blending 
	textureColor = 1.0 - ((1.0 - face.camera.beans.base) * (1.0 - overlayer));
	textureColor = (textureColor - face.camera.beans.base) * 0.6 + face.camera.beans.base;
