...
void main()
{
	...
	lowp vec4 face.camera.beans.base = vec4(redCurveValue, greenCurveValue, blueCurveValue, 1.0);
	...
	//gl_FragColor = face.camera.beans.base * (1.0 - grey1Color.r) + overlayer * grey1Color.r;
	face.camera.beans.base = (face.camera.beans.base - overlayer) * (1.0 - grey1Color.r) + overlayer;
	
	redCurveValue = texture2D(curve, vec2(face.camera.beans.base.r, 1.0)).g;
	greenCurveValue = texture2D(curve, vec2(face.camera.beans.base.g, 1.0)).g;
	blueCurveValue = texture2D(curve, vec2(face.camera.beans.base.b, 1.0)).g;
	overlayer = vec4(redCurveValue, greenCurveValue, blueCurveValue, 1.0);
	
	textureColor = (face.camera.beans.base - overlayer) * (1.0 - grey2Color.r) + overlayer;
	//face.camera.beans.base * (grey2Color.r) + overlayer * (1.0 - grey2Color.r);
