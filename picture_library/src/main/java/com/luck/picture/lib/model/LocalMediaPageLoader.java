...
public final class LocalMediaPageLoader {
    ...
    private static final List<String> SpecialFloder = new ArrayList() {
        {
            add("Camera");
            add("ZP");
        }
                    ...
                    if (data != null) {
                        ...
                        if (data.getCount() > 0) {
                            ...
                            do {
                                ...
                                Log.d("folderName", folderName + ">>" + size + ">>" + fileName + ">>" + bucket_id);
    ...
    public String setLatestCover(Context context, ImageView imageView) {
        final String[] coverPath = {null};
        loadAllMedia(new OnQueryDataResultListener() {
            @Override
            public void onComplete(List data, int currentPage, boolean isHasMore) {
                ...
                for (int i = 0; i < data.size(); i++) {
                    LocalMediaFolder mediaFolder = (LocalMediaFolder) data.get(i);
                    if (mediaFolder == null || mediaFolder.getName() == null) {
                        continue;
                    }
                    if (mediaFolder.getName().equals("ZP")) {
                        coverPath[0] = LocalMediaPageLoader
                                .getInstance(context, config).getFirstCover(mediaFolder.getBucketId());
                    }
                    if (mediaFolder.getName().equals("Camera")) {
                        tempCover = LocalMediaPageLoader
                                .getInstance(context, config).getFirstCover(mediaFolder.getBucketId());
                    }

//                mediaFolder.setFirstImagePath(firstCover);

                    if (coverPath[0] == null || coverPath[0].isEmpty()) {

                        coverPath[0] = tempCover;
                    }

                }
                if (PictureSelectionConfig.imageEngine != null && (coverPath[0] != null && !coverPath[0].isEmpty())) {
                    PictureSelectionConfig.imageEngine.loadFolderImage(context,
                            coverPath[0], imageView);
                }
